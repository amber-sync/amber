...
export const AppSettings: React.FC = () => {
  ...
  const themes: { id: Theme; label: string; color: string; description?: string }[] = [
    { id: 'system', label: 'System', color: 'linear-gradient(135deg, #ffffff 50%, #1f2937 50%)', description: 'Follow system' },
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {themes.map((t) => (
                    ...
                    >
                      ...
                      />
                      <span className={`text-xs font-medium ${theme === t.id ? 'text-accent-primary' : 'text-text-secondary'}`}>
                        {t.label}
                      </span>
                      {t.description && (
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 rounded-xl bg-layer-2 border border-border-base">
                  <div>
                    ...
                    <div className="text-sm text-text-secondary">Keep Amber running in the macOS menu bar when closed</div>
                  </div>
                  <input 
                    type="checkbox" 
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-layer-2 border border-border-base">
                  <div>
                    ...
                    <div className="text-sm text-text-secondary">Show desktop alerts for finished jobs</div>
                  </div>
                  <input 
                    type="checkbox" 
                    className="w-4 h-4 accent-accent-primary rounded cursor-pointer" 
