...
export const RestoreWizard: React.FC<RestoreWizardProps> = ({ job, onBack, onRestore }) => {
  ...
  const handleRestoreFull = async () => {
    if (!activeSnapshot) return;
    if (!confirm('Are you sure you want to restore the ENTIRE snapshot? This will copy all files to a "Restored" folder.')) return;
        ...
        <div className="w-80 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {sortedSnapshots.map(snap => (
              ...
              >
                <div className={`p-2 rounded-full ${selectedSnapshotId === snap.id ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'}`}>
                  <Icons.Clock size={16} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className={`text-sm font-medium ${selectedSnapshotId === snap.id ? 'text-blue-700 dark:text-blue-300' : 'text-gray-900 dark:text-gray-200'}`}>
                    {new Date(snap.timestamp).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}
                  </div>
          ...
          {activeSnapshot && snapshotPath ? (
            <div className="h-full flex flex-col gap-4">
               <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg p-3 flex items-start gap-3 text-sm text-blue-800 dark:text-blue-300">
                  ...
                  <div>
                    Viewing snapshot from <strong>{new Date(activeSnapshot.timestamp).toLocaleString()}</strong>.
                    Select files to restore them to the original location.
                  </div>
               </div>
